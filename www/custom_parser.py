    ...
    dumps(
        shoot(
            ...
            lambda data : mdict({
                ...
                'date_bandwidth': # mdict({str(data['_datetime']): 1 }),
                    mdict({str(dt.combine(data["_datetime"],datetime.time(0,0,0)).date()): int(data["bytes"]) }),

                'date_hit': mdict({str(data['_datetime'].date()): 1 }),
                    #mdict({str(dt.combine(data["_datetime"],datetime.time(0,0,0)).date()): 1 }),
                'hour_hit': mdict({"%2d" % int(data['_hour']): 1 }),
                'hour_bandwidth': mdict({"%2d" % int(data['_hour']): int(data["bytes"]) }),
